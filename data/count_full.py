...
def train(QList,AList,folder):
    ...
    #for i in rclist:
        ...
        full_texts.append(input_text)
        full_texts.append(target_text)
    for full_text in full_texts:
        for char in full_text.split():
    ...
    max_encoder_seq_length = max([len(txt) for txt in full_texts])
    max_decoder_seq_length = max([len(txt) for txt in full_texts])


    print('Number of samples:', len(full_texts))
    print('Number of unique input tokens:', num_encoder_tokens)
    print('Number of unique output tokens:', num_decoder_tokens)
    print('Max sequence length for inputs:', max_encoder_seq_length)
    print('Max sequence length for outputs:', max_decoder_seq_length)
    with open(folder + 'count.csv', 'w') as the_file:
        print('Number of samples:', len(full_texts),file=the_file)
